...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const userDto = await signup({ name: name.trim(), email: email.trim(), password });
      // Cache the name locally — the backend has no GET /users/me, and the
      // name is only ever returned once here (see utils/profileCache.js).
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Join Stayora to book and manage your stays.</p>
          </div>
        </div>
